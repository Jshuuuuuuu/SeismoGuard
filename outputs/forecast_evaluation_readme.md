...
- Forecast file: `forecast_archive/fc_ts_2026-05-20.csv`
...
4. Fill `actual_max_magnitude_7d` in `forecast_archive/eval_ts_2026-05-20_template.csv`.
5. Compute errors:
...
Short file naming convention for future runs:

- `fc_*`: frozen forecast
- `eval_*`: forecast evaluation
- `actual_*`: actual scraped events used in an evaluation
- `model_*`: trained model pipeline
- `X_*` / `y_*`: cached modeling matrix and target vector
- `ts`: two-stage no-event-aware model
